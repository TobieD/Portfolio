...
<style type="text/css">
  ...
  class PhotoTag < Liquid::Tag
    ...
    def render(context)
      ...
      if @filename
        "<div><a href=\"#{p.path_for(@filename)}\" class=\"fancybox\" title=\"#{@title}\"><img src=\"#{p.thumb_for(@filename,@thumb)}\" alt=\"#{@title}\" /></a></div>"
      else
  ...
  class VideoTag < Liquid::Tag
    ...
    def render(context)
      ...
      if @filename
        "<div>
          <a href=\"#{p.path_for(@filename)}\" class=\"fancybox fancybox.iframe\" title=\"#{@title}\" target=\"_blank\">
              <iframe frameborder= \"0\" controls= \"0\" showinfo= \"0\" allowfullscreen src=\"#{p.thumb_for(@filename,@thumb)}\" alt=\"#{@title}\"></iframe>
          </a>
        </div>"
      else
        "Error processing input, expected syntax: {% video filename [thumbnail] [title] %}"
      end
    end
  end
